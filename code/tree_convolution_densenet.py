...
class TreePruning(nn.Module):
    ...
    def gen_grouping(self, layer_name, group_num=8):
        if layer_name in self.wl.conv_names or layer_name in self.wl.fc_names:
            ...
            prev_conv_channels = filter_size[0]
            #print(filter_size)
            if conv_channels == 3:
                #input
                return None
                ...
                groups = []
                if conv_channels == prev_conv_channels:
                    ...
                    group_size = (conv_channels-prev_conv_channels+group_num-1) // group_num
                    permutation_idx = np.random.permutation(conv_channels-prev_conv_channels)
                    group_size2 = (prev_conv_channels+group_num-1) // group_num
                    permutation_idx2 = np.random.permutation(prev_conv_channels) + (conv_channels-prev_conv_channels)
                    for i in range(group_num):
                        groups.append(permutation_idx[group_size*i:group_size*(i+1)].tolist() + permutation_idx2[group_size2*i:group_size2*(i+1)].tolist())
                return np.array(groups)
    
    #To see whether no training works?
    def train_all(self, max_epoch=0):
        self.model.load_state_dict(torch.load('model_dense.pkl'))
    ...
    def train_layer(self, idx, max_epoch=1):
            ...
            for i in range(group_num):
                ...
                up_b = len(self.descendents[idx+1])
                for k in range(up_b):
                    descendent_set = self.descendents[idx+1][k]
                    total_acc = 0.0
                    mean_acc = 0.0
            ...
            for i in range(len(best_connections)):
                ...
                if len(self.descendents[idx+1][i]) == 0:
                    continue
                if len(layer_filter_size)==4:
                    #0105: Because DenseNet has skip connections.
                    if 'trans' not in layer_name:
                        for k in next_groups[i]:
                            if k > final_filter.size(1):
                                final_filter[k-final_filter.size(1),groups[best_connections[i]],:,:] = loaded_filter[k-final_filter.size(1),groups[best_connections[i]],:,:]
                    else:
                        for k in next_groups[i]:
    ...
    def train_layer_one_connection(self, idx, max_epoch=2, filter_mask=None):
        ...
        #VGG
        if self.model.net_type == 'Dense':
            if layer_type == "conv":
                if "trans" not in layer_name:
                    params = getattr(getattr(getattr(self.model.model, layer_name.split('.')[1]), layer_name.split('.')[2]), layer_name.split('.')[3]).parameters()
                else:
                    params = getattr(getattr(self.model.model, layer_name.split('.')[1]), layer_name.split('.')[2]).parameters()
            ...
            for idx_, (data, target) in enumerate(self.train_loader):
                ...
                if layer_type == 'fc':
                    ...
                    if "trans" not in layer_name:
                        ...
                        getattr(getattr(self.model.model, layer_name.split('.')[1]), layer_name.split('.')[2]).weight.grad *= filter_mask.cuda()
    ...
    def pretrain(self, max_epoch=160):
        ...
        for epoch in range(max_epoch):
            ...
            if acc > cur_acc:
                cur_acc = acc
                torch.save(self.model.state_dict(), 'model_dense.pkl')
    ...
    def retrain(self, max_epoch):
        ...
        def gen_filters():
            ...
            for layer_name in model_copy.state_dict():
                    if layer_name in self.wl.fc_names:
                        ...
                        elif 'trans' in layer_name:
                            filter_np =  getattr(getattr(model_copy.model, layer_name.split('.')[1]), layer_name.split('.')[2]).weight.cpu().detach().numpy()
                        else:
        ...
        self.model = self.model.cuda()
        self.model.load_state_dict(torch.load('checkpoints/model_layer_15.pkl'))
